...
def get_predictors_pandas(filtered_data):
    logistic_data = filtered_data[['rewarded', 'common_transitions', 'first_action', 'next_first_action']]
    logistic_data.insert(0, 'intercept', 1)
    logistic_data['rewarded'] = logistic_data['rewarded'].apply(encode)
    logistic_data['common_transitions'] = logistic_data['common_transitions'].apply(encode)
    logistic_data['interaction'] = logistic_data['rewarded'] * logistic_data['common_transitions']
    logistic_data['same_first_choice'] = np.where((filtered_data['first_action'] == filtered_data['next_first_action']), 1, -1)

    x = logistic_data[['intercept', 'rewarded', 'common_transitions', 'interaction']].values.tolist()
    y = logistic_data['same_first_choice'].values.tolist()
    return x, y
 
def objective(trial):
    w_local_inh = trial.suggest_float('w_local_inh', 0.0, 0.05)
    w_stn_inh = trial.suggest_float('w_stn_inh', 0.0, 0.5)
    w_inh_sd2 = trial.suggest_float('w_inh_sd2', 0.0, 0.3)
    w_inh_thal = trial.suggest_float('w_inh_thal', 0.0, 0.3)    
    ...
    k_dip_ITStrD2_shell = trial.suggest_float('k_dip_ITStrD2_shell', 0.0, 0.5)
    k_burst_ITStrD2_shell = trial.suggest_float('k_burst_ITStrD2_shell', 0.5, 2.0)
    tau_ITStrD2_shell = trial.suggest_int('tau_ITStrD2_shell', 0, 20)
    k_dip_ITSTN_shell = trial.suggest_float('k_dip_ITSTN_shell', 0.0, 0.5)
    k_burst_ITSTN_shell = trial.suggest_float('k_burst_ITSTN_shell', 0.5, 2.0)
    tau_ITSTN_shell = trial.suggest_int('tau_ITSTN_shell', 750, 3000)
    k_dip_StrD2VP_indir = trial.suggest_float('k_dip_StrD2VP_indir', 0.0, 0.2)
    k_burst_StrD2VP_indir = trial.suggest_float('k_burst_StrD2VP_indir', 0.5, 2.0)
    tau_StrD2VP_indir = trial.suggest_int('tau_StrD2VP_indir', 500, 2000)
    k_dip_STNVP_dir = trial.suggest_float('k_dip_STNVP_dir', 0.2, 0.8)
    k_burst_STNVP_dir = trial.suggest_float('k_burst_STNVP_dir', 0.5, 2.0)
    tau_STNVP_dir = trial.suggest_int('tau_STNVP_dir', 6000, 12000)
    k_dip_StrD1VP_dir = trial.suggest_float('k_dip_StrD1VP_dir', 0.4, 1.6)
    k_burst_StrD1VP_dir = trial.suggest_float('k_burst_StrD1VP_dir', 0.6, 2.4)
    tau_StrD1VP_dir = trial.suggest_int('tau_StrD1VP_dir', 500, 3000)
    k_dip_HippoRPELayerRPE = trial.suggest_float('k_dip_HippoRPELayerRPE', 0.025, 0.1)
    k_burst_HippoRPELayerRPE = trial.suggest_float('k_burst_HippoRPELayerRPE', 0.5, 2.0)
    tau_HippoRPELayerRPE = trial.suggest_int('tau_HippoRPELayerRPE', 0, 400)
    base_VP_dir = trial.suggest_float('base_VP_dir', 0.5, 3.0)
    tau_RPEVTA = trial.suggest_int('tau_RPEVTA', 0.0, 6000.0)

    StrD1StrD1_shell.w = w_local_inh
    RPERPE.w = w_local_inh
    ...
    for pop in ITSTN_shell:
        pop.K_dip = k_dip_ITSTN_shell
        pop.K_burst = k_burst_ITSTN_shell
        pop.tau = tau_ITSTN_shell
    ...
    for pop in STNVP_dir:
        pop.K_dip = k_dip_STNVP_dir
        pop.K_burst = k_burst_STNVP_dir
        pop.tau = tau_STNVP_dir
    
    for pop in STNSTN_shell:
        pop.w = w_stn_inh
    ...
    StrThalStrThal_shell.w = w_inh_thal
    VentralBG_VP_dir.baseline = base_VP_dir
    RPEVTA.tau = tau_RPEVTA

    HippoRPELayerRPE.K_dip = k_dip_HippoRPELayerRPE
    HippoRPELayerRPE.K_burst = k_burst_HippoRPELayerRPE
    ...
    for sim in range(sim_no):
        ...
        xx, yy = get_predictors_pandas(data)
    ...
    loss = mean_squared_error(true_target, probs)

    return loss

study = optuna.create_study(
    study_name='daw_logreg',

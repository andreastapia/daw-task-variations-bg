...
def objective(trial):
    ...
    tau_ITStrD1_shell = trial.suggest_int('tau_ITStrD1_shell', 100, 400)
    regularization_threshold_ITStrD1_shell = trial.suggest_float('regularization_threshold_ITStrD1_shell', 0.5, 2.0)
    tau_alpha_ITStrD1_shell = trial.suggest_float('tau_alpha_ITStrD1_shell', 1.0, 10.0)
    k_dip_ITStrD1_shell = trial.suggest_float('k_dip_ITStrD1_shell', 0.0, 0.1)
    k_burst_ITStrD1_shell = trial.suggest_float('k_burst_ITStrD1_shell', 0.5, 2.0)
    threshold_pre_ITStrD1_shell = trial.suggest_float('threshold_pre_ITStrD1_shell', 0.1, 0.4)
    threshold_post_ITStrD1_shell = trial.suggest_float('threshold_post_ITStrD1_shell', 0.0, 1.0)
    
    tau_ITStrD2_shell = trial.suggest_int('tau_ITStrD2_shell', 5.0, 20.0)
    regularization_threshold_ITStrD2_shell = trial.suggest_float('regularization_threshold_ITStrD2_shell', 0.5, 2.0)
    tau_alpha_ITStrD2_shell = trial.suggest_float('tau_alpha_ITStrD2_shell', 10.0, 30.0)
    k_dip_ITStrD2_shell = trial.suggest_float('k_dip_ITStrD2_shell', 0.1, 0.4)
    k_burst_ITStrD2_shell = trial.suggest_float('k_burst_ITStrD2_shell', 0.5, 2.0)
    threshold_pre_ITStrD2_shell = trial.suggest_float('threshold_pre_ITStrD2_shell', 0.1, 0.3)
    threshold_post_ITStrD2_shell = trial.suggest_float('threshold_post_ITStrD2_shell', 0.001, 0.009)
    
    tau_StrD2VP_indir = trial.suggest_int('tau_StrD2VP_indir', 500, 1500)
    regularization_threshold_StrD2VP_indir = trial.suggest_float('regularization_threshold_StrD2VP_indir', 1.0, 3.0)
    tau_alpha_StrD2VP_indir = trial.suggest_float('tau_alpha_StrD2VP_indir', 10.0, 30.0)
    k_dip_StrD2VP_indir = trial.suggest_float('k_dip_StrD2VP_indir', 0.05, 0.2)
    k_burst_StrD2VP_indir = trial.suggest_float('k_burst_StrD2VP_indir', 0.5, 2.0)
    threshold_pre_StrD2VP_indir = trial.suggest_float('threshold_pre_StrD2VP_indir', 0.1, 0.5)
    threshold_post_StrD2VP_indir = trial.suggest_float('threshold_post_StrD2VP_indir', 0.0, 0.1)
    
    w_StrD2StrD2_shell = trial.suggest_float('w_StrD2StrD2_shell', 0.0, 0.5)
    w_StrD1StrD1_shell = trial.suggest_float('w_StrD1StrD1_shell', 0.0, 0.05)

    tau_StrD1VP_dir = trial.suggest_int('tau_StrD1VP_dir', 1000, 3000)
    regularization_threshold_StrD1VP_dir = trial.suggest_float('regularization_threshold_StrD1VP_dir', 2.0, 5.0)
    tau_alpha_StrD1VP_dir = trial.suggest_float('tau_alpha_StrD1VP_dir', 1.0, 20.0)
    k_dip_StrD1VP_dir = trial.suggest_float('k_dip_StrD1VP_dir', 0.5, 1.5)
    k_burst_StrD1VP_dir = trial.suggest_float('k_burst_StrD1VP_dir', 0.8, 2.5)
    threshold_pre_StrD1VP_dir = trial.suggest_float('threshold_pre_StrD1VP_dir', 0.1, 0.4)
    threshold_post_StrD1VP_dir = trial.suggest_float('threshold_post_StrD1VP_dir', 0.2, 0.6)
    
    w_VPVP_shell = trial.suggest_float('w_VPVP_shell', 0.4, 1.2)
    w_ThalThal = trial.suggest_float('w_ThalThal', 0.0, 0.5)
    w_VP_indirVP_dir = trial.suggest_float('w_VP_indirVP_dir', 0.2, 0.8)
    w_StrThalVP_indir = trial.suggest_float('w_StrThalVP_indir', 0.1, 0.5)
    w_StrThalVP_dir = trial.suggest_float('w_StrThalVP_dir', 0.5, 1.5)
    w_VPThal = trial.suggest_float('w_VPThal', 0.4, 1.5)

    tau_HippoRPELayerRPE = trial.suggest_int('tau_HippoRPELayerRPE', 0, 400)
    tau_RPEVTA = trial.suggest_int('tau_RPEVTA', 0.0, 6000.0)
    ...
    for pop in ITStrD2_shell:
        pop.tau = tau_ITStrD2_shell
        pop.regularization_threshold = regularization_threshold_ITStrD2_shell
        pop.tau_alpha = tau_alpha_ITStrD2_shell
        pop.K_dip = k_dip_ITStrD2_shell
        pop.K_burst = k_burst_ITStrD2_shell
        pop.threshold_pre = threshold_pre_ITStrD2_shell
        pop.threshold_post = threshold_post_ITStrD2_shell
    
    for pop in StrD2VP_indir:
        pop.tau = tau_StrD2VP_indir
        pop.regularization_threshold = regularization_threshold_StrD2VP_indir
        pop.tau_alpha = tau_alpha_StrD2VP_indir
        pop.K_dip = k_dip_StrD2VP_indir
        pop.K_burst = k_burst_StrD2VP_indir
        pop.threshold_pre = threshold_pre_StrD2VP_indir
        pop.threshold_post = threshold_post_StrD2VP_indir

    for pop in StrD2StrD2_shell:
        pop.w = w_StrD2StrD2_shell
    ...
    VP_indirVP_dir.w = w_VP_indirVP_dir
    StrThalVP_indir.w = w_StrThalVP_indir
    ...
    for sim in range(sim_no):
        ...
        for i in range(4):
            reward_of_actions.append(get_random_reward())
        ...
        for trial in range(num_trials):
            ...
            # change the rewards
            for i in range(4):
                reward_swap_random=random.random()
                if reward_swap_random < reward_change_prob:
                    reward_of_actions[i] = get_random_reward()
    ...
    true_target = [0.92, 0.8, 0.65, 0.72]
    loss = mean_squared_error(true_target, avg_probs)

    return loss

study = optuna.create_study(
    study_name='bg_bigger_dir_indir',
    storage='mysql://optuna:123456@localhost:3306/optuna_dez',
    load_if_exists=True
)

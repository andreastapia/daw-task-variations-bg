...
import os
import datetime
def doll_task_ventral_fixed():
    print("DOLL TASK FINAL TEST")
    now = datetime.datetime.now()
    result_path = 'basal_ganglia/results/doll_task_ventral_fixed_double_2/simulation_{date}/'.format(date=now.strftime("%Y%m%d%H%M%S"))
    ...
    selected_objectives=[]
    achieved_sequences=[]
    reward_of_actions=[0.25,0.75,0.75,0.25]
    #reward_of_actions=[]
    dopamine_activities=[]
    ...
    for trial in range(num_trials):
        ...
        #else, keep it because it's doable

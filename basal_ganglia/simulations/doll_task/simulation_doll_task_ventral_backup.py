...
StrD1VP_dir.connect_all_to_all(weights=Normal(0.25,0.01)) # scale by numer of stimuli #var 0.01
StrD1VP_dir.tau = 2000 #700 #550
StrD1VP_dir.regularization_threshold = 4.0 #2 #3.5 #1.5
StrD1VP_dir.tau_alpha = 10.0 # 20.0
StrD1VP_dir.baseline_dopa = baseline_dopa_vent
StrD1VP_dir.K_dip = 1.0 #0.9
StrD1VP_dir.K_burst = 1.6
StrD1VP_dir.threshold_post = 0.25 #0.1 #0.3
StrD1VP_dir.threshold_pre = 0.05 # 0.15
StrD1VP_dir.DA_type=1
StrD1VP_dir.negterm = 5.0

# also plastic connections for the Cells encoding the reward prediction
HippoRPE = Projection(pre=Hippocampus,post=HippoActivity[:12],target='exc')
...
VPVP_shell.connect_all_to_all(weights=1.0)#0.7#0.2
...
StrThalVP_dir.connect_one_to_one(weights=1.1) #1.1

VPThal = Projection(pre=VentralBG_VP_dir,post=VentralBG_Thal,target='inh')
VPThal.connect_one_to_one(weights=0.55) # 1.5
...
import os
import datetime
def doll_task_ventral():
    now = datetime.datetime.now()
    result_path = 'basal_ganglia/results/doll_task_ventral/simulation_{date}/'.format(date=now.strftime("%Y%m%d%H%M%S"))
    ...
    np.save(result_path + "strd2_activities.npy",strd2_activities)

...
for i in range(0,num_stim):
    ...
    ITStrD1_shell[i].tau = 262 #100
    ITStrD1_shell[i].regularization_threshold = 1.954869220148737
    ITStrD1_shell[i].tau_alpha = 6.163036849814096
    ITStrD1_shell[i].baseline_dopa = baseline_dopa_vent
    ITStrD1_shell[i].K_dip = 0.037855189141766055
    ITStrD1_shell[i].K_burst = 1.2193284373759852
    ITStrD1_shell[i].DA_type = 1
    ITStrD1_shell[i].threshold_pre = 0.32555531235120766
    ITStrD1_shell[i].threshold_post = 0.25428977543252446
    ...
    ITStrD2_shell[i].tau = 16
    ITStrD2_shell[i].regularization_threshold = 1.6309671025725585
    ITStrD2_shell[i].tau_alpha = 16.662269143556937
    ITStrD2_shell[i].baseline_dopa = baseline_dopa_vent
    ITStrD2_shell[i].K_dip = 0.1794993887599337
    ITStrD2_shell[i].K_burst = 1.9883545509803484
    ITStrD2_shell[i].DA_type = -1
    ITStrD2_shell[i].threshold_pre = 0.2717775163651044
    ITStrD2_shell[i].threshold_post = 0.0010418088838206998
    ...
    StrD2VP_indir[i].tau = 970 
    StrD2VP_indir[i].regularization_threshold = 2.5623403129718034
    StrD2VP_indir[i].tau_alpha = 26.65216521625525
    StrD2VP_indir[i].baseline_dopa = baseline_dopa_vent
    StrD2VP_indir[i].K_dip = 0.1184464606801104
    StrD2VP_indir[i].K_burst = 1.639269341887293
    StrD2VP_indir[i].threshold_pre = 0.2822570307120158
    StrD2VP_indir[i].threshold_post = 0.09920701342615054
    ...
    StrD2StrD2_shell[i].connect_all_to_all(weights= 0.4945201652457672)
...
StrD1StrD1_shell.connect_all_to_all(weights = 0.004071472343811361) 
...
StrD1VP_dir.tau = 2684 #700 #550
StrD1VP_dir.regularization_threshold = 3.787670637574928 #2 #3.5 #1.5
StrD1VP_dir.tau_alpha = 10.539343778756734 # 20.0
StrD1VP_dir.baseline_dopa = baseline_dopa_vent
StrD1VP_dir.K_dip = 1.1785492635025157 #0.9
StrD1VP_dir.K_burst = 2.028535869553606
StrD1VP_dir.threshold_pre = 0.18735165353867209 # 0.15
StrD1VP_dir.threshold_post = 0.2004313006194597 #0.1 #0.3
...
def dezfoulli_task_ventral_optuna():
    now = datetime.datetime.now()
    result_path = 'basal_ganglia/results/dezfoulli_task_ventral_optuna/simulation_{date}/'.format(date=now.strftime("%Y%m%d%H%M%S"))
    ...
    #each second stage option has either a high or low reward probability
    for i in range(4):
    ...
    np.save(result_path + "swapped_probs.npy",swapped_probs)

...
for i in range(0,num_stim):
    ...
    ITStrD1_shell[i].tau = 399 #100
    ITStrD1_shell[i].regularization_threshold = 1.8136889156905132
    ITStrD1_shell[i].tau_alpha = 2.607973199557397
    ITStrD1_shell[i].baseline_dopa = baseline_dopa_vent
    ITStrD1_shell[i].K_dip = 0.05740196542818797
    ITStrD1_shell[i].K_burst = 1.9350868417365537
    ITStrD1_shell[i].DA_type = 1
    ITStrD1_shell[i].threshold_pre = 0.1480028575491138
    ITStrD1_shell[i].threshold_post = 0.36848258844881554
    ...
    ITStrD2_shell[i].regularization_threshold = 0.7279503250556404
    ITStrD2_shell[i].tau_alpha = 27.00154872587069
    ITStrD2_shell[i].baseline_dopa = baseline_dopa_vent
    ITStrD2_shell[i].K_dip = 0.16663701773625722
    ITStrD2_shell[i].K_burst = 1.5026949122639275
    ITStrD2_shell[i].DA_type = -1
    ITStrD2_shell[i].threshold_pre = 0.12549957130157793
    ITStrD2_shell[i].threshold_post = 0.0056719513678756255
    ...
    StrD2VP_indir[i].tau = 1371 
    StrD2VP_indir[i].regularization_threshold = 2.0632328556914463
    StrD2VP_indir[i].tau_alpha = 27.117873908382837 
    StrD2VP_indir[i].baseline_dopa = baseline_dopa_vent
    StrD2VP_indir[i].K_dip = 0.18030198513634427
    StrD2VP_indir[i].K_burst = 1.1030325353621528
    StrD2VP_indir[i].threshold_post = 0.08390793442199825
    StrD2VP_indir[i].threshold_pre = 0.42898974132442785
    ...
    StrD2StrD2_shell[i].connect_all_to_all(weights= 0.34942472006599873)
...
StrD1StrD1_shell.connect_all_to_all(weights = 0.004869772099725313) 
...
StrD1VP_dir.tau = 1218 #700 #550
StrD1VP_dir.regularization_threshold = 2.5184109495445877 #2 #3.5 #1.5
StrD1VP_dir.tau_alpha = 15.433473590837156 # 20.0
StrD1VP_dir.baseline_dopa = baseline_dopa_vent
StrD1VP_dir.K_dip = 0.9415000026462473 #0.9
StrD1VP_dir.K_burst = 1.903545524125048
StrD1VP_dir.threshold_post = 0.3334101209021228 #0.1 #0.3
StrD1VP_dir.threshold_pre = 0.30968353933009757 # 0.15
...
import os
import datetime
def daw_task_ventral_optuna():
    now = datetime.datetime.now()
    result_path = 'basal_ganglia/results/daw_task_ventral_optuna/simulation_{date}/'.format(date=now.strftime("%Y%m%d%H%M%S"))

...
# geometry is the total number of neurons in the hippocampus
HippoActivity= Population(name='HippoActivity',geometry=32,neuron=LinearNeuron)
...
StrD1VP_dir.connect_all_to_all(weights=Normal(0.27,0.01)) # scale by numer of stimuli #var 0.01
...
#TODO:ajustar pesos para que se active el mPFC
...
VPThal.connect_one_to_one(weights=0.85) # 1.5
...
for trial in range(num_trials):
    ...
    for stim in stimulis:   
        ...
        Hippocampus[:,stim//2].baseline=1.0 
        Hippocampus[:,4+stim%4].baseline=1.0
    ...
    # if first_panel == 0:
    #     objs = list(range(4))
    #     selected_objective=random.choices(objs,weights=(mPFC[:4].r+eps),k=1)[0]
    # elif first_panel == 1:
    #     objs = list(range(4,8))
    #     selected_objective=random.choices(objs,weights=(mPFC[4:].r+eps),k=1)[0]
    ...
    #0.425
    #get value from 0-3
    first_action = selected_objective // 2
    #get value from 0-3
    ...
    realizable = False
    if first_action not in first_choices:
        first_action = random.choice(first_choices)
    ...
    #if second action is not in the panel
    #else, keep it because is doable
    if second_action not in second_choices:
        second_action = random.choice(second_choices)
...
ax1.plot(np.arange(mpfc_act_np.shape[1]), mpfc_act_np[trial_pick][:,0], label='(F1,B1)')
ax1.plot(np.arange(mpfc_act_np.shape[1]), mpfc_act_np[trial_pick][:,1], label='(F1,B2)')
ax1.plot(np.arange(mpfc_act_np.shape[1]), mpfc_act_np[trial_pick][:,2], label='(F2,S1)')
ax1.plot(np.arange(mpfc_act_np.shape[1]), mpfc_act_np[trial_pick][:,3], label='(F2,S2)')
ax1.plot(np.arange(mpfc_act_np.shape[1]), mpfc_act_np[trial_pick][:,4], label='(T1,B1)')
ax1.plot(np.arange(mpfc_act_np.shape[1]), mpfc_act_np[trial_pick][:,5], label='(T1,B2)')
ax1.plot(np.arange(mpfc_act_np.shape[1]), mpfc_act_np[trial_pick][:,6], label='(T2,S1)')
ax1.plot(np.arange(mpfc_act_np.shape[1]), mpfc_act_np[trial_pick][:,7], label='(T2,S2)')
...
ax2.plot(np.arange(vp_dir_trial_avg.shape[0]), vp_dir_trial_avg[:,0], label='(F1,B1)')
ax2.plot(np.arange(vp_dir_trial_avg.shape[0]), vp_dir_trial_avg[:,1], label='(F1,B2)')
ax2.plot(np.arange(vp_dir_trial_avg.shape[0]), vp_dir_trial_avg[:,2], label='(F2,S1)')
ax2.plot(np.arange(vp_dir_trial_avg.shape[0]), vp_dir_trial_avg[:,3], label='(F2,S2)')
ax2.plot(np.arange(vp_dir_trial_avg.shape[0]), vp_dir_trial_avg[:,4], label='(T1,B1)')
ax2.plot(np.arange(vp_dir_trial_avg.shape[0]), vp_dir_trial_avg[:,5], label='(T1,B2)')
ax2.plot(np.arange(vp_dir_trial_avg.shape[0]), vp_dir_trial_avg[:,6], label='(T2,S1)')
ax2.plot(np.arange(vp_dir_trial_avg.shape[0]), vp_dir_trial_avg[:,7], label='(T2,S2)')
